<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl flex justify-between items-center text-gray-800 leading-tight">
            {{ __('Categorie') }}
            <a href="{{ route('categories.create') }}" class="float-right bg-[#2A3F54] text-[#fff] px-4 py-2 rounded-md font-normal text-sm">Create</a>
        </h2>
    </x-slot>
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b">
                                    ...
                                    <td class="px-6 py-2 flex gap-2 border-b-2">
                                        @can('edit_permission')
                                        <a class="bg-green-600 text-white px-2 py-1 rounded-md" href="{{ route('categories.edit',$categorie) }}"><i class="fa-solid fa-pen-to-square"></i></a>
                                        @endcan
                                        @can('delete_permission')
                                        <form action="{{  route('categories.destroy', $categorie) }}" method="POST" onsubmit="return confirm('Are you sure?')" class="inline-block">
                                           @method('DELETE')
                                           @csrf
